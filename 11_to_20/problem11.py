...
def largestDiagR(grid):
    grt = 0
    for i in range(GRID_SIZE-3):
        for j in range((GRID_SIZE-1), 3, -1):
            prod = grid[i][j] * grid[i+1][j-1] * grid[i+2][j-2] * grid[i+3][j-3]
            if prod > grt:
                grt = prod
    return grt
...
def largest(grid):
    return max([largestSide(grid), largestUp(grid), largestDiagL(grid), largestDiagR(grid)])
